...
public class GPhoto2 {
	 ...
	 */
	public GPhoto2() {
		gphoto2 = (Gphoto2Library) Native.loadLibrary("gphoto2", Gphoto2Library.class);
	 ...
	 * <p>
	 * After a 'capture', some cameras will return immediately but before the image
	 * has been generated. Enable waitForCaptureEvent() to ensure the image is
	 * ready.
	 * </p>
	 * <p>
	 * Other cameras block internally until the image is ready, and therefore don't
	 * generate capture events. These devices will timeout if waitForCaptureEvent()
	 * is enabled.
	 * </p>
	 *
	 * @param wait When true, capture methods will block until the camera returns a
	 *             Capture or Timeout event.
	 */
	...
	public List<Camera> listCameras() throws IOException {
		...
		/*
		 * This is an rather absurd amount of code for what it actually accomplishes in
		 * the end.
		 */
		int rc;
		...
		for (int i = 0; i < size; i++) {
			...
			/*
			 * Get List of Abilities for this Model, then associate the Abilities with a
			 * Camera
			 */
	 ...
	 *
	 * @param camera Specify the camera to open by passing one of the Camera objects
	 *               obtained by {@link #listCameras()}.
	 * @throws IOException
	 */
...
//==================================================================================
	...
	/**
	 * Counts the choices of a CameraWidget.
	 *
	 * @param widget a CameraWidget of type GP_WIDGET_RADIO or GP_WIDGET_MENU
	 * @return a gphoto2 error code or number of choices.
	 */
	private int getNumWidgetChoices(CameraWidget widget) {
		return gphoto2.gp_widget_count_choices(widget);
	}
	 ...
	 *
	 * @param widget    a CameraWidget of type GP_WIDGET_RADIO or GP_WIDGET_MENU
	 * @param choiceNum the numerical value of a choice
	 * @return a string value for the given choice
	 */
	private String getChoice(CameraWidget widget, int choiceNum) {
		String[] choiceText = new String[1];
		gphoto2.gp_widget_get_choice(widget, choiceNum, choiceText);
		return choiceText[0];
	}

	/**
	 * Returns an ArrayList of possible choices for the given widget.
	 *
	 * @param widget    a CameraWidget of type GP_WIDGET_RADIO or GP_WIDGET_MENU
	 * @return an ArrayList of strings
	 */	
	public List<String> getChoiceList(CameraWidget widget) {
		int numChoices = getNumWidgetChoices(widget);
		ArrayList<String> list = new ArrayList<String>();

		for (int k = 0; k < numChoices; k++) {
			list.add(getChoice(widget, k));
		}
		return list;
	}
...
//==================================================================================
	...
	/**
	 * Take a picture and save it to disk. Currently images can only be saved into
	...
	/**
	 * Fire the shutter, then download all images on the camera. Useful when using
	 * burstmode, which may capture multiple images for one captureImage() call.
	 *
	...
	/**
	 * Change a single camera setting. If updating several parameters at once, it
	 ...
	 * @throws IOException
	 */
	public void setConfig(GPhoto2Config config, String param, String value) throws IOException {
		config.setParameter(param, value);
		config.writeConfig();
	}

	/**
	 * Read a single camera setting. If reading several parameters at once it may be
	...
	/**
	 * Save image to disk in current directory. TODO: allow path and filename to be
	 * specified.
	 *
	...
	/**
	 * Download all images in the given path into the current directory. Warning: If
	 * delete option is enabled, all images on the camera will be deleted, not just
	 * the ones from the most recent capture.
	 *
